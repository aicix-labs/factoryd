...
- **"A second pass with agent CLIs changes nothing except who writes the code"
  ...
  code from whether `$FACTORYD_PROGRESS` moved **and** reaps agent helpers
  before it returns; the wrapper is tested for each way it can be wrong
  (`TestTurnWrapperDerivesTheExitCodeFromProgress`).
