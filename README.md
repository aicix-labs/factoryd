...
files, the gate, no git or network, the progress marker, and on a verdict the
family to re-declare verbatim — so a brief only describes work (SPEC §6.2).
...
refresh --config <f>` by hand, `--force` to start a cycle regardless). Nothing
else brings a credential-less, network-less producer forward.
